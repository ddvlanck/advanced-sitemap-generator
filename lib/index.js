...
module.exports = function SitemapGenerator(uri, opts) {
  ...
  const detectUrlLang = (urlObj) => {
    const init = (resolve, reject) => {
      request({
        ...
        urlObj.lang = $('html').attr('lang') ? $('html').attr('lang') : '';
  ...
  crawler.on('fetchcomplete', (queueItem, page) => {
    const { url } = queueItem;
    let urlObj = {value: url, flushed: false, alternatives: [], lang: 'en'};
    ...
    if (sitemaps.length > 1) {
      ...
      each(
        ...
        (item, done) => {
          ...
          let postfix = options.ignoreHreflang ? '' : `_${item.lang}`;
          postfix += !item.onlyMe ? `_part${count[item.lang]}` : '';
          const newPath = extendFilename(sitemapPath, postfix);
          paths.push(newPath);

          // copy and remove tmp file
          cpFile(item.path, newPath).then(() => {
            fs.unlink(item.path, () => {
              done();
            });
          });
        ...
        () => {
          ...
          fs.writeFile(
            sitemapPath,
            createSitemapIndex(parsedUrl.toString(), filename, count, options),
            cb
          );
        }
      );
      ...
      paths.unshift(sitemapPath);
      cpFile(sitemaps[0].path, sitemapPath).then(() => {
        fs.unlink(sitemaps[0].path, cb);
      });
    } else {
      cb();
    }
