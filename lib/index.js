...
module.exports = function SitemapGenerator(uri, opts) {
  ...
  const getStats = () => ({
    ...
    maxReachedCrawlDepth: maxReachedCrawlDepth
  });
  ...
  const addURL= (url, depth) => {
    ...
    if(depth > maxReachedCrawlDepth){
      maxReachedCrawlDepth = depth;
    }
  ...
  const onCrawlerComplete = () => {
    const init = () => {
      console.log(cachedResultURLs.length);
      sitemap.flush();
      sitemap.finish();
